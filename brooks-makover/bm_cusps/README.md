...
Version 0.4.4 writes a `run_parameters` block (`n`, `seed`, `h`, `L0`, `T_ext`, `neig`, `W`) and a `quality` block to every result.  Ensemble analyses accept only records with `quality.analysis_eligible=true`.  Non-converged compact solves remain available as diagnostics but are excluded from statistics.  SLURM restart validation checks all numerical parameters plus schema/code/graph-feature provenance and convergence status.  Rejected cached files are moved to `results/quarantine/` with a `.json.quarantine` suffix so they cannot match the normal `results/*.json` glob.

### Result quality in 0.4.4

Quality is recomputed when a JSON result is read; the stored `quality` block is not trusted as an acceptance certificate.  The branches are independent:

- `compact_analysis_eligible`: compactified spectrum is finite and the Liouville solve satisfies its recorded tolerance.
- `h4_analysis_eligible`: compact branch plus finite, positive horocycle-profile data actually used to infer `c0`.
- `cusped_analysis_eligible`: the DtN branch is internally consistent; `lambda1_cusped=null` is valid only for the successful `no_l2_detected` status.
- `full_analysis_eligible`: both compact and cusped branches are valid.

Failed DtN root searches may retain `lambda1_cusped_raw` for diagnostics, but this raw value is never exported as an accepted cusp eigenvalue.  `aggregate.py` exports the branch-specific status columns and blanks invalid cusp values.

The files in `results_example/` are historical regression fixtures whose metadata was migrated to the current schema.  They carry `provenance.numerical_payload_recomputed=false`; do not use them as native 0.4.4 campaign results or paper data.
